...
10. 邮箱池列表支持点击邮箱复制、标签/备注双行居中展示、备注与状态公共弹窗快捷编辑，以及带二次确认、持续进度提示和结果统计的串行彻底删除队列。
...
- `/mailboxes`：按单个或全部 Apple 账号同步已有隐私邮箱、搜索、点击复制、备注/状态快捷编辑、详情、邮件、取码、远端清理、串行远程删除和本地删除。
...
- 邮箱池每次彻底删除均需二次确认；连续删除按队列串行执行，当前项与等待项分别显示“删除中”和“排队中”，顶部提示持续到队列成功或失败后再结束。
- 备注更新使用可选字段区分“未提交”和“提交空值”，列表快捷编辑可真正清空已有备注。
...
- Vue 生产构建、`go test ./...`、`go vet ./...`、`go build ./...` 均已通过。
...
- 已增加存储层回归测试，覆盖备注写入、未提交时保持原值和提交空值后清空。
