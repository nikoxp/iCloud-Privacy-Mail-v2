...
公共取号 API 说明与“生成新 Key”按钮同行，`/verification-code` 的“打开页面”链接靠最右侧。邮箱池分页数量由页面自动计算，因此设置页不再提供每页数量字段。
...
页面使用独立布局，不显示后台导航。管理员开启“公共验证码页面”后，外部用户可以输入完整邮箱地址查询最近验证码。当前默认匹配 OpenAI 关键词并使用 5 分钟新鲜窗口。
